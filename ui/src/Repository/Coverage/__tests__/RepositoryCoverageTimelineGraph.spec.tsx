...
describe("RepositoryCoverageTimelineGraph", () => {
  ...
  it("should display coverage timeline graph", async () => {
    ...
    expect(lineDot1).toHaveAttribute("aria-label", "dot-lineValue-89.01");

    const lineDot2 = await waitFor(() => getByRole(`dot-lineValue-COV2`), {
      timeout: 10000,
    });
    expect(lineDot2).toHaveAttribute("aria-label", "dot-lineValue-90.02");

    const lineDot3 = await waitFor(() => getByRole(`dot-lineValue-COV3`), {
      timeout: 10000,
    });
    expect(lineDot3).toHaveAttribute("aria-label", "dot-lineValue-91.03");

    const branchDot1 = await waitFor(() => getByRole(`dot-branchValue-COV1`), {
      timeout: 10000,
    });
    expect(branchDot1).toHaveAttribute("aria-label", "dot-branchValue-90.01");

    const branchDot2 = await waitFor(() => getByRole(`dot-branchValue-COV2`), {
      timeout: 10000,
    });
    expect(branchDot2).toHaveAttribute("aria-label", "dot-branchValue-91.02");

    const branchDot3 = await waitFor(() => getByRole(`dot-branchValue-COV3`), {
      timeout: 10000,
    });
    expect(branchDot3).toHaveAttribute("aria-label", "dot-branchValue-92.03");
  });
});
