...
class GitHubCommentService(private val commentClient: GitHubCommentClient) {
    ...
    fun upsertReportComment(commentData: ReportCommentData, publicId: String): PullRequest? {
        ...
        return if (repository != null) {
            logger.info("Searching for pull request with pull request number $pullRequestNumber branch name $branchName or Git commit SHA $commitSha in repo $orgName/$repoName for public ID $publicId")
            ...
            val prNumber = if (pullRequestNumber != null) {
                ...
                logger.info("Need branch name or pull request number to find pull request in repo $orgName/$repoName for public ID $publicId")
                null
            }
            ...
            if (prNumber != null) {
                ...
                logger.info("Could not find pull request for branch $branchName or commit SHA $commitSha in repo $orgName/$repoName for public ID $publicId")
                null
            }
        } else {
            null
