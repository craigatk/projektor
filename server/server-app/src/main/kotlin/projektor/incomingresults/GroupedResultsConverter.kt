...
class GroupedResultsConverter(
    ...
    suspend fun parseAndConvertGroupedResults(groupedResultsBlob: String): GroupedResults =
        withContext(Dispatchers.IO) {
            val incomingGroupedResults = groupedResultsParser.parseGroupedResults(groupedResultsBlob)

            val groupedTestSuites =
                incomingGroupedResults.groupedTestSuites?.map {
                    val nonEmptyTestSuites =
                        testResultsProcessor.parseResultsBlob(it.testSuitesBlob)
            ...
            val metadata =
                incomingGroupedResults.metadata?.let { metadata ->
                    ResultsMetadata(
                        git =
                            metadata.git?.let { gitMetadata ->
                                GitMetadata(
            ...
            val performanceResults =
                incomingGroupedResults.performanceResults?.mapNotNull { perfResults ->
            ...
            GroupedResults(
                ...
                coverageFiles =
                    incomingGroupedResults.coverageFiles?.map { incomingCoverageFile ->
                        CoverageFilePayload(
                ...
                codeQualityReports =
                    incomingGroupedResults.codeQualityFiles?.mapIndexed { idx, codeQualityReport ->
                        CodeQualityReport(
                            idx = idx + 1,
