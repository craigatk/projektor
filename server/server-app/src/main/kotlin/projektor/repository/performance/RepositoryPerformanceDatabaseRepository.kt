...
class RepositoryPerformanceDatabaseRepository(private val dslContext: DSLContext) : RepositoryPerformanceRepository {
    private val repositoryPerformanceTimelineEntryMapper =
        JdbcMapperFactory.newInstance()
            .addKeys("public_id")
            .ignorePropertyNotFound()
            .newMapper(RepositoryPerformanceTestTimelineEntry::class.java)

    override suspend fun fetchTestTimelineEntries(
        repoName: String,
        projectName: String?,
    ): List<RepositoryPerformanceTestTimelineEntry> =
        withContext(Dispatchers.IO) {
            ...
            val timelineEntries: List<RepositoryPerformanceTestTimelineEntry> =
                resultSet.use {
